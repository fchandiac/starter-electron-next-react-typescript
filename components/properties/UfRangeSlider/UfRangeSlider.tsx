...
const UfRangeSlider: React.FC<RangeSliderProps> = ({ min = 0, max = 100, value, onChange }) => {
    ...
    const formatUF = (num: number) => `UF ${num.toLocaleString('es-CL')}`;
        ...
        <div className="w-full pr-4">
            <div className="flex items-center gap-1" >
                ...
                <span className="text-xs text-foreground font-light">
                    Rango en UF ({values[0].toLocaleString('es-CL')} - {values[1].toLocaleString('es-CL')})
                </span>
            </div>
